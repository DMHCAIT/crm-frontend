...
const LeadsManagement: React.FC = () => {
  ...
  const [createdLeadFilter, setCreatedLeadFilter] = useState('all');
  const [createdLeadDateType, setCreatedLeadDateType] = useState('on');
  const [createdLeadDate, setCreatedLeadDate] = useState('');
  const [createdLeadDateFrom, setCreatedLeadDateFrom] = useState('');
  const [createdLeadDateTo, setCreatedLeadDateTo] = useState('');
  ...
  useEffect(() => {
    applyFilters();
  }, [leads, searchQuery, dateFilter, dateFrom, dateTo, createdLeadFilter, createdLeadDateType, createdLeadDate, createdLeadDateFrom, createdLeadDateTo, statusFilter, countryFilter, sourceFilter, assignedToFilter, leadsUpdatedToday]);

  // Reset to first page when filters change
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, dateFilter, dateFrom, dateTo, createdLeadFilter, createdLeadDateType, createdLeadDate, createdLeadDateFrom, createdLeadDateTo, statusFilter, countryFilter, sourceFilter, assignedToFilter]);
  ...
  const applyFilters = () => {
    ...
    // Created Lead Date Filter
    if (createdLeadFilter !== 'all') {
      filtered = filtered.filter(lead => {
        const leadCreatedDate = new Date(lead.createdAt);
        const leadCreatedDateOnly = new Date(leadCreatedDate.getFullYear(), leadCreatedDate.getMonth(), leadCreatedDate.getDate());
        
        switch (createdLeadDateType) {
          case 'on':
            if (createdLeadDate) {
              const targetDate = new Date(createdLeadDate);
              const targetDateOnly = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate());
              return leadCreatedDateOnly.getTime() === targetDateOnly.getTime();
            }
            return true;
          case 'after':
            if (createdLeadDate) {
          ...
          case 'before':
            ...
            return true;
          case 'between':
            if (createdLeadDateFrom && createdLeadDateTo) {
              const fromDate = new Date(createdLeadDateFrom);
              const toDate = new Date(createdLeadDateTo);
              const fromDateOnly = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate());
              const toDateOnly = new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate());
              return leadCreatedDateOnly.getTime() >= fromDateOnly.getTime() && 
                     leadCreatedDateOnly.getTime() <= toDateOnly.getTime();
            }
            return true;
          default:
            return true;
  ...
  const handleSelectAll = () => {
    const currentPageLeads = getPaginatedLeads();
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {loading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 flex items-center space-x-4">
            ...
            <span className="text-gray-700">Loading leads...</span>
          </div>
        </div>
      )}
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Created Lead Filter</label>
                <select
                  value={createdLeadFilter}
                  onChange={(e) => setCreatedLeadFilter(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="all">All Leads</option>
                  <option value="filtered">Filter by Creation Date</option>
                </select>
              </div>

              {createdLeadFilter === 'filtered' && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Date Type</label>
                    <select
                      value={createdLeadDateType}
                      onChange={(e) => setCreatedLeadDateType(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                  ...
                  {(createdLeadDateType === 'on' || createdLeadDateType === 'after' || createdLeadDateType === 'before') && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Date</label>
                      <input
                        type="date"
                        value={createdLeadDate}
                        onChange={(e) => setCreatedLeadDate(e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                  )}

                  {createdLeadDateType === 'between' && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">From Date</label>
                        <input
                          type="date"
                          value={createdLeadDateFrom}
                          onChange={(e) => setCreatedLeadDateFrom(e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="date"
                          value={createdLeadDateTo}
                          onChange={(e) => setCreatedLeadDateTo(e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                      </div>
                    </>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="flex items-center space-x-3">
                  <input
                    ...
                    checked={(() => {
                      const currentPageLeads = getPaginatedLeads();
                      const currentPageLeadIds = currentPageLeads.map(lead => lead.id);
                      return currentPageLeadIds.length > 0 && currentPageLeadIds.every(id => selectedLeads.includes(id));
                    })()}
                    onChange={handleSelectAll}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-sm text-gray-600">
                    {selectedLeads.length > 0 ? `${selectedLeads.length} selected` : `Select all on page`}
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
              <div className="text-sm text-gray-700">
                Showing {filteredLeads.length === 0 ? 0 : ((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, filteredLeads.length)} of {filteredLeads.length} leads
              </div>
              ...
              {getTotalPages() > 1 && (
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex items-center space-x-1">
                    {Array.from({ length: Math.min(getTotalPages(), 7) }, (_, index) => {
                      let pageNum;
                      if (getTotalPages() <= 7) {
                        pageNum = index + 1;
      ...
      {filteredLeads.length > 0 && getTotalPages() > 1 && (
        <div className="mt-6 flex items-center justify-center">
          <div className="flex items-center space-x-2">
            <button
              onClick={() => handlePageChange(currentPage - 1)}
              disabled={currentPage === 1}
              className="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1"
            >
              <span>Previous</span>
            </button>
