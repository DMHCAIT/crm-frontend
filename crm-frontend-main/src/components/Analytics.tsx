...
const Analytics: React.FC = () => {
  ...
  const leadSourcesData = useMemo((): LeadSourceData[] => {
    console.log('📈 Analytics: Processing lead sources');
    const startTime = performance.now();
    ...
    filteredLeads.forEach((lead: any) => {
      ...
      if (lead.status === 'Enrolled' || lead.status === 'won') {
        current.converted += 1;
        current.revenue += lead.fees || lead.value || lead.actualRevenue || 0;
      } else if ((lead.status === 'Warm' || lead.status === 'Hot') && lead.estimatedValue) {
        current.revenue += lead.estimatedValue * 0.3; // 30% probability
      }
  ...
  const analyticsData = useMemo((): AnalyticsData => {
    ...
    filteredLeads.forEach((lead: any) => {
      ...
      if (lead.status === 'Enrolled' || lead.status === 'won') {
        convertedCount++;
        revenue = lead.fees || lead.value || lead.actualRevenue || 0;
      } else if ((lead.status === 'Warm' || lead.status === 'Hot') && lead.estimatedValue) {
